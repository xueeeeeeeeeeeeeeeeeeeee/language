<template>
  ...
  let { course } = toRefs(props);
  watch(course, async (val) => {
      console.log(course._object.course);
  ...
  const adddata = () => {
    ...
    for (var i = 0; i < rowSelection.value.selectedRowKeys.length; i++) {
      console.log(data.value[rowSelection.value.selectedRowKeys[i]].UnitID);
      bindTi(data.value[rowSelection.value.selectedRowKeys[i]].UnitID);
    }
    message.info("绑定成功");
  };
  const bindTi=async(x)=>{
      ...
      formData.append("ExamUnitID", x);
      formData.append("CourseID",course._object.course);
      await axios({
          method: "post",
          url: "/exam/BindExamUnitWithCourse",
          data: formData,
          headers: {
