<template>
    <a-drawer
      ...
      :width="720"
    >
      <addecoursesection :CourseID="nowCourseID" :data="indata" :statechange="statechange"></addecoursesection>
    </a-drawer>
    <div class="index">
      <div class="rightbody">
        <div class="index-mainbody">
          <a-button @click="adddata">新建小节</a-button>
    ...
    <script setup>
  import addecoursesection from "@/components/addecoursesection";
...
init();
  ...
  const init=()=>{    nowCourseID.value=CourseID._object.CourseID;
  ...
  const selectTi = async (x) => {
    ...
    await axios({
      method: "post",
      url: "/courseunit/getcourseunitbyparameters",
      data: formData,
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }).then(function (response) {
      res = response.data.sections[0];
    });
